...
What it does
- Reads ~/.openclaw/openclaw.json (or a path you pass).
- Adds a provider named "clausy" pointing at your Clausy server (OpenAI-compatible).
- Sets Clausy as the primary/default model.
- Preserves all existing configuration.
- If an existing primary/default model is found, it is moved into an alias entry
  so you can switch back easily.

This script is intentionally conservative: it only adds/updates the minimal keys it needs,
and never deletes unknown keys.

Usage
  python scripts/openclaw_install_clausy.py
  python scripts/openclaw_install_clausy.py --config ~/.openclaw/openclaw.json
  python scripts/openclaw_install_clausy.py --base-url http://127.0.0.1:3108/v1 --model chatgpt-web

After running, restart OpenClaw components if needed (gateway/UI/agent).
"""
...
def _detect_primary(cfg: Dict[str, Any]) -> Tuple[Optional[Dict[str, Any]], str]:
    ...
    Returns (primary_object, location_string). If not found, (None, "").

    We keep this heuristic because OpenClaw config schemas can evolve.
    We never *remove* anything; we only copy the detected primary into aliases.
    """
    # Most-likely locations first:
    # 1) cfg["models"]["default"] = {"provider": "...", "model": "..."} (dict)
...
def _install(cfg: Dict[str, Any], base_url: str, model_id: str, provider_name: str = "clausy") -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # Save previous primary as alias (if it exists and isn't already Clausy)
    if old_primary:
        # detect if old already points to clausy
    ...
    # Add/Update Clausy provider.
    # Keep it generic: OpenAI-compatible provider with baseURL.
    providers[provider_name] = {
        "type": "openai",
        "baseURL": base_url,
        # Some systems require a key field even for local servers; use a placeholder.
        "apiKey": "clausy",
    }

    # Set default/primary in the most common place: models.default
...
def main() -> int:
    ...
    ap.add_argument("--config", default=DEFAULT_CONFIG, help="Path to openclaw.json (default: ~/.openclaw/openclaw.json)")
    ap.add_argument("--base-url", default=DEFAULT_BASE_URL, help="Clausy OpenAI base URL (default: http://127.0.0.1:3108/v1)")
    ap.add_argument("--model", default=DEFAULT_MODEL_ID, help="Model id to set as default (default: chatgpt-web)")
